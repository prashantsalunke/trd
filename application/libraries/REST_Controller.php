...
abstract class REST_Controller extends MX_Controller {
	 ...
	 * an "object".
	 * This simply maps the object and method to the correct
	 * Controller method
	 *
	 * @access public
	 ...
	 *        	HTTP status code
	 * @param bool $continue
	 *        	TRUE to flush the response to the client and continue
	...
	/**
	 * Takes mixed data and optionally a status code, then creates the response
	 * within the buffers of the Output class.
	...
	/**
	 * Get the HTTP request string e.g.
	 * get or post
	 *
	 * @access protected
	...
	protected function _check_limit($controller_method) {
		...
		$result = $this->rest->db->where ( 'uri', $limited_uri )->where ( 'api_key', $this->rest->key )->get ( $this->config->item ( 'rest_limits_table' ) )->row ();
		...
		elseif ($result->hour_started < (time () - $time_limit)) {
			...
			$this->rest->db->where ( 'uri', $limited_uri )->where ( 'api_key', isset ( $this->rest->key ) ? $this->rest->key : '' )->set ( 'hour_started', time () )->set ( 'count', 1 )->update ( $this->config->item ( 'rest_limits_table' ) );
		} 		
		...
		// They have called within the hour, so lets update
			...
			$this->rest->db->where ( 'uri', $limited_uri )->where ( 'api_key', $this->rest->key )->set ( 'count', 'count + 1', FALSE )->update ( $this->config->item ( 'rest_limits_table' ) );
		}
			...
			if (! empty ( $auth_override_class_method [$this->router->class] ['*'] )) // Check for class overrides
{
	 ...
	 * @access public
	 * @param NULL $key
	 *        	Key to retrieve from the GET request
	 ...
	 * @access public
	 * @param NULL $key
	 *        	Key to retrieve from the OPTIONS request.
	 *        	If NULL an array of arguments is returned
	 * @param NULL $xss_clean
	 *        	Whether to apply XSS filtering
	 ...
	 * @access public
	 * @param NULL $key
	 *        	Key to retrieve from the HEAD request
	 *        	If NULL an array of arguments is returned
	 * @param NULL $xss_clean
	 *        	Whether to apply XSS filtering
	 ...
	 * @access public
	 * @param NULL $key
	 *        	Key to retrieve from the POST request
	 *        	If NULL an array of arguments is returned
	 * @param NULL $xss_clean
	 *        	Whether to apply XSS filtering
	 ...
	 * @access public
	 * @param NULL $key
	 *        	Key to retrieve from the PUT request
	 *        	If NULL an array of arguments is returned
	 * @param NULL $xss_clean
	 *        	Whether to apply XSS filtering
	 ...
	 * @access public
	 * @param NULL $key
	 *        	Key to retrieve from the DELETE request
	 *        	If NULL an array of arguments is returned
	 * @param NULL $xss_clean
	 *        	Whether to apply XSS filtering
	 ...
	 * @access public
	 * @param NULL $key
	 *        	Key to retrieve from the PATCH request
	 *        	If NULL an array of arguments is returned
	 * @param NULL $xss_clean
	 *        	Whether to apply XSS filtering
	 ...
	 * @access public
	 * @param NULL $key
	 *        	Key to retrieve from the query parameters
	 *        	If NULL an array of arguments is returned
	 * @param NULL $xss_clean
	 *        	Whether to apply XSS filtering
